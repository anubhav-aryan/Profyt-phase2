...
export function LeakageForm({ onComplete }: Props) {
  ...
  const [notes, setNotes] = useState<Record<string, string>>(
    () => Object.fromEntries(BUCKETS.map((b) => [b.id, ""])) as Record<string, string>
  );
  ...
  async function submitAssessment() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/portal/assessments/leakage", {
        method: "POST",
  ...
  return (
    <div className="profyt-card max-w-2xl p-8 md:p-10">
      <div className="mb-8 border-b border-pale pb-6">
        <h1 className="font-serif text-3xl text-black">Leakage Map</h1>
      ...
      <div className="space-y-8">
        ...
        {isLastQuestionInBucket ? (
          <div>
            <label className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-dark">
              Notes for this bucket (optional)
            </label>
            <textarea
              className="profyt-input mt-2 min-h-[100px] resize-y"
              value={notes[currentBucket.id] ?? ""}
              onChange={(e) =>
                setNotes((prev) => ({
                  ...prev,
                  [currentBucket.id]: e.target.value,
                }))
              }
              disabled={submitting}
            />
